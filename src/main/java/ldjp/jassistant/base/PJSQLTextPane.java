...
import ldjp.jassistant.db.format.BlancoSqlFormatter;
import ldjp.jassistant.db.format.BlancoSqlRule;
...
public class PJSQLTextPane extends JTextPane {
    ...
    public void toUpperCase() {
        ...
        if (!StringUtil.isEmptyWithTrim(selText)) {
        ...
        if (para != null) {
            ...
            for (int i = 0; i < s.length(); i++) {
                ...
                if (curChar == '-'
                        && i < s.length() - 1
                        && s.charAt(i + 1) == '-') {
                    doc.setCharacterAttributes(start + i, s.length() - i,
                ...
                if (wordBegin >= 0 && wordEnd > wordBegin) {
                    ...
                    if (wordBegin > 0 && wordEnd < s.length()
                            ...
                            && s.charAt(wordEnd) == '\'') {
                        doc.setCharacterAttributes(start + wordBegin - 1, wordEnd
                                - wordBegin + 2, getStyle("quoted"), false);
                    } else if (isSQL92Keyword(oneWord)) {
                        doc.setCharacterAttributes(start + wordBegin, wordEnd
                                - wordBegin, getStyle("keyword"), false);
                    } else if (isTableName(oneWord)) {
                        doc.setCharacterAttributes(start + wordBegin, wordEnd
                                - wordBegin, getStyle("tablename"), false);
                    } else if (isFunction(oneWord)) {
                        if (wordEnd < s.length()) {
                            char nextChar = s.charAt(wordEnd);
                            if (nextChar == '(' || nextChar == ' ') {
                                doc.setCharacterAttributes(start + wordBegin, wordEnd
                                        - wordBegin, getStyle("function"), false);
                            }
                        }
                    }
