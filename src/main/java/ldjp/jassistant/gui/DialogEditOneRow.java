...
public class DialogEditOneRow extends PJDialogBase {
    ...
    ImageIcon iconRefreshRecord = ImageManager
            .createImageIcon(PJConst.IMAGE_REFRESHTABLE);
	int tableType = PJConst.TABLE_TYPE_TABLE;
	Vector<String> nameVector = null;
	Vector<String> commentVector = null;
	Vector<Object> rowData = null;
	Vector<Class<?>> typeVector = null;
	Vector<Integer> sizeVector = null;
	Vector<Boolean> keyVector = null;
	Vector<PJEditorTextField> editorVector = null;
	int row = -1;
	...
	public DialogEditOneRow() {
		this(Main.getMDIMain(), WordManager.getWord(WordKeyConsts.W0036), true);
	}
	...
	void jbInit() throws Exception {
		...
		btnRefresh.setToolTipText(WordManager.getWord(WordKeyConsts.W0037));
	...
	void refreshData() {
		...
		editorVector = new Vector<PJEditorTextField>(columnCounts);
	...
	void updateRecord() {
		...
		try {
			Vector<Object> newRowData = new Vector<Object>(columnCounts + 1);
			for (int i = 0; i < columnCounts; i++) {
				Class<?> type = typeVector.get(i);
				Object value = rowData.get(i);
			...
			if (shouldUpdate) {
                fireParamTransferEvent(new Object[] { new Integer(row), newRowData },
                        PJConst.WINDOW_EDITONEROW);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 * init ui from data
	 */
	public void initUI(Vector<String> nameVector, Vector<String> commentVector, Vector<Class<?>> typeVector,
						Vector<Integer> sizeVector, Vector<Boolean> keyVector,
						Vector<Object> rowData, int row, int tableType) {
		...
		if (PJConst.TABLE_TYPE_TABLE != tableType) {
			...
			setTitle(WordManager.getWord(WordKeyConsts.W0038));
		}
