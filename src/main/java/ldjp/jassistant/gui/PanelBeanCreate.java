...
import ldjp.jassistant.base.FileClassLoader;
...
import ldjp.jassistant.util.TemplateManager;
...
public class PanelBeanCreate extends JPanel implements Refreshable {
	...
	JCheckBox chkToClipboard = new JCheckBox(WordManager.getWord(WordKeyConsts.W0070));
	...
	void jbInit() throws Exception {
		...
		lblPackageName.setText(WordManager.getWord(WordKeyConsts.W0162));
		...
		btnCreate.setText(WordManager.getWord(WordKeyConsts.W0163));
		...
		lblDescription.setText(WordManager.getWord(WordKeyConsts.W0164));
		...
		btnImportFromFile.setText(PJConst.EMPTY);
		btnImportFromFile.setIcon(iconImportFromFile);
		btnImportFromFile.addActionListener(buttonMenuActionListener);
		btnImportFromFile.setToolTipText(WordManager.getWord(WordKeyConsts.W0165));
		btnSaveToFile.setText(PJConst.EMPTY);
		btnSaveToFile.setIcon(iconSaveToFile);
		btnSaveToFile.addActionListener(buttonMenuActionListener);
		btnSaveToFile.setToolTipText(WordManager.getWord(WordKeyConsts.W0166));
		btnAddColumn.setText(PJConst.EMPTY);
		btnAddColumn.setIcon(iconAddColumn);
		btnAddColumn.addActionListener(buttonMenuActionListener);
		btnAddColumn.setToolTipText(WordManager.getWord(WordKeyConsts.W0167));
		btnRefresh.setToolTipText(WordManager.getWord(WordKeyConsts.W0168));
		btnRefresh.setText(PJConst.EMPTY);
		btnRefresh.setIcon(iconRefresh);
		btnRefresh.setEnabled(false);
		btnRefresh.addActionListener(buttonMenuActionListener);
		btnUpMove.setToolTipText(WordManager.getWord(WordKeyConsts.W0169));
		...
		btnDownMove.setToolTipText(WordManager.getWord(WordKeyConsts.W0170));
		btnDownMove.setActionCommand("Down");
		btnDownMove.setIcon(iconDownarrow);
		btnRemoveColumn.setText(PJConst.EMPTY);
		...
		btnRemoveColumn.setToolTipText(WordManager.getWord(WordKeyConsts.W0170));
	...
	String currentBeanType = PJConst.EMPTY;
	DBDescTableModel dataModel = new DBDescTableModel();
    String[] tableColumnHeaders = new String[] { " ",
            WordManager.getWord(WordKeyConsts.W0172),
            WordManager.getWord(WordKeyConsts.W0173),
            WordManager.getWord(WordKeyConsts.W0174),
            "Getter","Setter",
            WordManager.getWord(WordKeyConsts.W0092),
            WordManager.getWord(WordKeyConsts.W0175)
             };
	int needColumn = 7;
	...
	private void initTableColumn() {
        ...
        tblColumnDesc.getColumnModel().getColumn(BUTTON_COLUMN[0])
                .setCellRenderer(buttonCellRender);
        tblColumnDesc.getColumnModel().getColumn(LABELTEXT_COLUMN[0])
                .setCellRenderer(labelCellRender);
        tblColumnDesc.getColumnModel().getColumn(LABELTEXT_COLUMN[1])
                .setCellRenderer(labelCellRender);
	}
	...
	public void setTableColumnDescData(ArrayList<ColumnDescriptionData> listData) {
		...
		if (listData != null && !listData.isEmpty()) {
			...
			for (int i=0; i<listData.size(); i++) {
                ...
                String columnName = convertColumnName(oneData.getColumnName(),
                        PropertyManager.getProperty(PJConst.OPTIONS_COLUMN_NAME_CASE));
                String columnClass = convertColumnClass(
                        oneData.getColumnType(),
                        oneData.getPrecision() > 0 ? oneData.getPrecision() : oneData
                                .getColumnSize());
                String columnComment = convertComment(oneData.getComment(), columnName,
                        PropertyManager
                                .getProperty(PJConst.OPTIONS_COLUMN_COMMENT_DEFAULT));

				if (isPrimaryKey) {
                    ImageIcon iconPrimaryKey = ImageManager
                            .createImageIcon("primarykey.gif");
	...
	private void addDefaultRow() {
		...
		vecOneRecord.add("null");
        if (PJConst.OPTIONS_TRUE.equals(PropertyManager
                .getProperty(PJConst.OPTIONS_COLUMN_METHOD_GETTER))) {
			vecOneRecord.add(new Boolean(true));
		} else {
			vecOneRecord.add(new Boolean(false));
		}
        if (PJConst.OPTIONS_TRUE.equals(PropertyManager
                .getProperty(PJConst.OPTIONS_COLUMN_METHOD_SETTER))) {
			vecOneRecord.add(new Boolean(true));
		} else {
			vecOneRecord.add(new Boolean(false));
		}
        ...
        if (selectedRow >= 0
                && tblColumnDesc.getSelectedColumns()[0] == BUTTON_COLUMN[0]) {
	...
	/**
	 * Convert column name label to preference case
	 *
	 * @param label column name
	 * @param flag 1: no convert, 2: to LOWWERCASE, 3: to UPPERCASE
	 *
	...
	public void refreshDisplay() {
        if (StringUtil.isEmpty(currentTableName)
                || currentBeanType.equals(PJConst.BEAN_TYPE_NEWBEAN)) {
			btnRefresh.setEnabled(false);
		} else {
    ...
    void importFromFile() {
		...
		try {
			...
			if (fileName.endsWith(".bean")) {
                Vector<Vector<Object>> preTableData = (Vector<Vector<Object>>) FileManager
                ...
                ArrayList<ColumnDescriptionData> columnListData =
                        new ArrayList<ColumnDescriptionData>();
	...
	class DBDescTableModel extends AbstractTableModel {
		...
		public void setValueAt(Object aValue, int row, int column) {
			tableData.get(row).set(column, aValue);
		}
		...
		public void rellocateIndexColumn() {
			for (int i = 0; i < tableData.size(); i++) {
			    Vector<Object> vecRec = tableData.get(i);
		...
		public void moveRow(int row, boolean up) {
			Vector<Object> vecRec = tableData.get(row);

			fireTableRowsUpdated(0, tableData.size());

			if (up) {
				if (row > 0) {
					Vector<Object> toVecRec = tableData.get(row - 1);
	...
	class ButtonCellRender extends JToggleButton implements javax.swing.table.TableCellRenderer {
        ...
        Border rollBorder = BorderFactory.createBevelBorder(BevelBorder.RAISED,
                Color.white, SystemColor.control, SystemColor.control, new Color(103,
                        101, 98));
	...
	private void selectionChanged() {
        ...
        if (selectedRows.length == 1 && selectedColumns.length > 0
                && selectedColumns[0] == BUTTON_COLUMN[0]) {
        ...
        if (selectedRows.length > 0
                ...
                && (selectedColumns[0] == LABELTEXT_COLUMN[0]
                    || selectedColumns[0] == LABELTEXT_COLUMN[1])) {
	...
	/**
	 * menu edit and tool bar copy, cut, paste, delete
	 * enabled or disabled
	 */
	...
	private synchronized void copy() {
		...
		for (int i = 0; i < selectedRows.length; i++) {
            String columnValue = (String) dataModel.getValueAt(selectedRows[i],
                    selectedColumns[0]);
			if (columnValue == null) {
				columnValue = PJConst.EMPTY;
			}
	...
	private synchronized void paste() {
		...
		try {
            pasteData = (String) clipboard.getContents(this).getTransferData(
		...
		if (selectedRows.length > 1) {
			...
			for (int i=0; i<selectedRows.length; i++) {
				if (i < copiedLineCounts) {
					dataModel.setValueAt(copiedLinesData[i], selectedRows[i],
					        selectedColumns[0]);
				}
			}
		} else {
			...
			for (int i=0; i<copiedLineCounts; i++) {
				if (firstSelectedRow + i < allRows) {
					dataModel.setValueAt(copiedLinesData[i], firstSelectedRow + i,
					        selectedColumns[0]);
				}
			}
		}
	...
	/**
	 * Translate copied data into String[]
	 */
