...
public class PanelSQLResult extends JPanel {
    ...
    void jbInit() throws Exception {
        ...
        tblScriptResult.getSelectionModel().addListSelectionListener(
    ...
    public boolean processResultShow(String name, String desc, String sql) {
        ...
        try {
            ...
            if (value instanceof DispatchModel) {
                ...
                DBTableModel dataModel = new DBTableModel(tblScriptResult, null, data,
    ...
    public int exportToExcel(Sheet s, int startRow, boolean isExportComment) {
        ...
        if (!StringUtil.isEmptyWithTrim(desc)) {
        ...
        if (model instanceof DBTableModel) {
            DBTableModel dbModel = (DBTableModel) model;
            Vector<String> columnRemark = dbModel.getColumnComment();
            Vector<String> columnName = dbModel.getRealColumnName();
            Vector<Class<?>> typeData = dbModel.getColumnJavaType();
            ...
            for (int i = 0; i < rowCount; i++) {
                r = s.createRow(startRow++);
                Vector<Object> rowData = dbModel.getRowData(i);
                for (int j = 0; j < columnName.size(); j++) {
                    String strValue = StringUtil.getStringValue(typeData.get(j),
                            rowData.get(j), false);
