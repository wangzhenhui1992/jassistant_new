...
public class DialogSaveTableData extends PJDialogBase {
	...
	public DialogSaveTableData() {
		this(Main.getMDIMain(), WordManager.getWord(WordKeyConsts.W0123), true);
	}
	...
	void jbInit() throws Exception {
        ...
        titledBorderSaveTo = new TitledBorder(BorderFactory.createEtchedBorder(
                Color.white, new Color(148, 145, 140)),
                WordManager.getWord(WordKeyConsts.W0121));
		...
		rdoSQL.setText(WordManager.getWord(WordKeyConsts.W0074));
    ...
    String getOneLine(Vector<String> columnNameVector, Vector<Object> dataVector,
		...
		if (rdoCommarDelim.isSelected()) {
            ...
            return StringUtil.joinSqlString(columnNameVector, dataVector, typeVector,
                    sizeVector, tableName,
	...
	class ThreadDoInsert extends Thread {
		public void run() {
			...
			try {
                int totalSize = DBParser.getTotalRowCount(Main.getMDIMain()
                        .getConnection(), tableName, Main.getMDIMain().currentConnURL);
				progressBarSave.setMaximum(totalSize);

                Object[] ret = DBParser.getTableData(Main.getMDIMain().getConnection(),
                        tableName, Main.getMDIMain().currentConnURL, null, null);
				rs = (ResultSet) ret[0];
				DispatchModel data = (DispatchModel) ret[1];

				Vector<String> columnNameVector = data.getClmNms();
				if (isShowComment) {
					columnNameVector = data.getCmts();
				}
				Vector<Class<?>> typeVector = data.getTypes();
				Vector<Integer> sizeVector = data.getSizes();
				Vector<Boolean> keyVector = data.getKeys();
				...
				if (chkIncludeHeader.isSelected()) {
					if (!rdoSQL.isSelected()) {
                        String headerLine = getOneLine(columnNameVector, null,
                                typeVector, sizeVector, true);
						appendOneLine(out, sb, headerLine);
					}
				}

				int count = 0;
				for (Vector<Object> oneRow : data.getVals()) {
				...
				while (true) {
					...
					for (int i = 0; i < moreData.size(); i++) {
						Vector<Object> oneRow = moreData.get(i);
                        String oneLine = getOneLine(columnNameVector, oneRow,
                                typeVector, sizeVector, false);
						appendOneLine(out, sb, oneLine);
						progressBarSave.setValue(++count);
				...
				try {
                    SwingUtilities.invokeAndWait(new InvokeAndWaitDoThread("MCSTC202E",
                            se.getMessage()));
					return;
				} catch (Exception ite) {}
			} catch (IOException e) {
