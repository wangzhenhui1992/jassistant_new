...
import ldjp.jassistant.base.PJInternalFrame;
...
public class FrmMain extends JFrame implements ParamTransferListener {
	...
	private void jbInit() throws Exception {
        ...
        mainStatusBar.setBorder(BorderFactory.createBevelBorder(BevelBorder.RAISED,
	...
	String extTitile = PJConst.EMPTY;
	...
	class ButtonMenuActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
            ...
            if (obj == menuBarMain.mnuFileItemNewJavaBean
                    || obj == mainToolBar.toolBarNewJavaBean) {
				createNewBean();
            } else if (obj == menuBarMain.mnuFileItemOpen
                    || obj == mainToolBar.toolBarOpenConnection) {
				showOpenConnectionDialog();
            } else if (obj == menuBarMain.mnuFileItemClose
                    || obj == mainToolBar.toolBarCloseConnection) {
            ...
            } else if (obj == menuBarMain.mnuViewItemOption
                    || obj == mainToolBar.toolBarOptions) {
				showOptionsDialog();
			} else if (obj == menuBarMain.mnuHelpItemAbout) {
				showAboutDialog();
            } else if (obj == menuBarMain.mnuDatabaseItemSchemaBrowser
                    || obj == mainToolBar.toolBarSchemaBrowser) {
				showDataBrowse();
            } else if (obj == menuBarMain.mnuDatabaseItemSQLEdit
                    || obj == mainToolBar.toolBarSQLEdit) {
				showSQLScript();
	 ...
	 */
	void createNewBean() {
        String beanName = MessageManager.showInputDialog("Please input bean name.",
                "Need bean Name");

		if (!StringUtil.isEmptyWithTrim(beanName)) {
			showDataBrowse();
	...
	void showDataBrowse() {
		if (jiSQLBrowser == null || jiSQLBrowser.isClosed()) {
            ...
            jiSQLBrowser = new PJInternalFrame(WordManager.getWord(WordKeyConsts.W0130),
                    sqlBrowserPanel, iconSchemaBrowse, menuBarMain.mnuWindow);
	...
	void arrangeInternalFrames(String index) {
		...
		for (int i = 0; i < frames.length; i++) {
			...
			if ("0".equals(index)) {
                ...
                oneFrame.setBounds(i * ((int) width / frames.length), 0,
                        (int) (width / frames.length), height);
			} else if ("2".equals(index)) {
                oneFrame.setBounds(0, i * ((int) height / frames.length), width,
	 ...
	 * Clear or initialize table names and functions used to highlight
	 */
