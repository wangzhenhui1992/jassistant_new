...
public class PanelDataHistory extends JPanel implements Refreshable {
    ...
    String[] historyHeaders = new String[] { "No",
            WordManager.getWord(WordKeyConsts.W0193),
            WordManager.getWord(WordKeyConsts.W0194) };
    int[] historyWidth = new int[]{40, 160, 300};
    Vector<Vector<Object>> vecHistoryData = new Vector<Vector<Object>>();
    int IDX_HISTNDX = 0;
    int IDX_HISTTIM = 1;
    int IDX_HISTMEM = 2;
    ...
    void jbInit() throws Exception {
        ...
        btnImport.setText(WordManager.getWord(WordKeyConsts.W0196));
    ...
    private void initPanelHistory() {
        ...
        panelHistory
                .setBorder(new TitledBorder(WordManager.getWord(WordKeyConsts.W0197)));
        ...
        btnDeleteHistory.setToolTipText(WordManager.getWord(WordKeyConsts.W0198));
        ...
        tblHistory.getSelectionModel().setSelectionMode(
    ...
    private void initPanelResult() {
        ...
        panelResult.setBorder(new TitledBorder(WordManager.getWord(WordKeyConsts.W0198)));
    ...
    */
    ...
    class HistoryTableModel extends AbstractTableModel {
        ...
        public Object getValueAt(int row, int col) {
            return vecHistoryData.get(row).get(col);
        }
        ...
        public void setValueAt(Object aValue, int row, int column) {
            vecHistoryData.get(row).set(column, aValue);
        }
        ...
        public void moveRow(int row, boolean up) {
            Vector<Object> vecRec = vecHistoryData.get(row);

            fireTableRowsUpdated(0, vecHistoryData.size());

            if (up) {
                if (row > 0) {
                    Vector<Object> toVecRec = vecHistoryData.get(row - 1);
