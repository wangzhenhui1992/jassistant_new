...
import ldjp.jassistant.model.BeanData;
import ldjp.jassistant.model.BeanFieldData;
...
public class TemplateManager {
	 ...
	 * import classes map
	 */
	HashMap<String,Object> importClassesMap = new HashMap<String,Object>();

	/**
	 * paren panel
	 */
	String packageName = PJConst.EMPTY;

	/**
	 * class description
	 */
	String classDescription = PJConst.EMPTY;
    ...
    public TemplateManager(String packageName, String classDescription,
            String configPath, Vector<Vector<Object>> tableData, File beanFile) {
	...
	/**
	 * Initialize bean data from properties and tables
	 */
	private void initBeanData() {
		...
		if (packageName.trim().length() > 0) {
			...
			beanData.setPackageDefine(PJConst.EMPTY);
		}
		...
		if (beanFile != null) {
		    ...
		    beanData.setClassName(PJConst.EMPTY);
		}
		...
		if (PJConst.OPTIONS_TRUE.equals(PropertyManager.getProperty(PJConst.OPTIONS_COLUMN_BEANINFO_SERIALIZABLE))) {
			...
			beanData.setSerializable(PJConst.EMPTY);
		}
	 ...
	 */
	private ArrayList<BeanFieldData> getBeanFieldDataList() {
		ArrayList<BeanFieldData> columnDataList = new ArrayList<BeanFieldData>();

		for (int i=0; i<tableData.size(); i++) {
			Vector<Object> vecOneColumn = (Vector<Object>) tableData.get(i);
			...
			if (classPos > 0) {
				...
				if (!"java.lang".equals(importPackage)) {
					importClassesMap.put(columnType, PJConst.EMPTY);
				}
	...
	private String getImportClasses() {
		...
		Iterator<String> it = importClassesMap.keySet().iterator();
		while (it.hasNext()) {
			String strImportClass = it.next();
	...
	private String processLoopColumnReplace(String templateContent) throws IllegalArgumentException {
		...
		ArrayList<BeanFieldData> columnList = beanData.getBeanFieldDataList();
		int contentLength = templateContent.length();
